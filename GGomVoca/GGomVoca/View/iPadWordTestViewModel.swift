...
final class iPadWordTestViewModel: ObservableObject {
    ...
    // MARK: - 시험지 생성
    func createPaper(vocabularyID: Vocabulary.ID, isMemorized: Bool) {
        self.getVocabulary(vocabularyID: vocabularyID)
        for word in words {
            if isMemorized {
                // 모든 단어 시험지에 추가
